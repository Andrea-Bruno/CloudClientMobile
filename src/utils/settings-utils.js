...
export const shareFile = async () => {
    ...
    if (!source) return store.dispatch(openModal({
        ...
        try {
            await Share.open({
                url: source,
                title: `Shareing ${editedFile.name} from Cloud Storage`
            });
        }
        catch {
...
export const onlyPick = async (closeBottomSheet) => {
    ...
    try {
        ...
        if (pickedFile.size > store.getState().profile.totalMemory) {
            return store.dispatch(openModal({
                content: 'Cloud Storage memory is already full. Delete unnecessary data to upload a new file.',
                type: 'info',
                head: "Cloud Storage is full",
                icon: 'ex',
            }))
        }
